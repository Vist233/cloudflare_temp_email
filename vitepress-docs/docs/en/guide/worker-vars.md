...
| Variable Name                         | Type      | Description                                                                                          | Example   |
| ------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------- | --------- |
| `ZHANG_AUTH_URL`                      | Text      | Zhang Auth OIDC issuer; must be `https://auth.zhangyvjing.com`. Sign-in uses authorization code + PKCE and never forwards a user password | `https://auth.zhangyvjing.com` |
| `ZHANG_AUTH_CLIENT_ID`                | Text      | Static first-party OIDC client ID; its callback is fixed at `https://your-domain/user_api/oidc/callback` | `tmpmail-web` |
| `ZHANG_AUTH_CLIENT_SECRET`            | Secret    | OIDC client secret. Set only with `wrangler secret put`; never put it in config or admin OAuth2 settings | - |
| `TMPMAIL_OIDC_COOKIE_SECRET`          | Secret    | Signs the 10-minute OIDC state/nonce/PKCE transaction cookie and 60-second sign-in handoff cookie; generate with `openssl rand -base64 48` | - |
...
> - With Zhang Auth configured, sign-in starts at `GET /user_api/oidc/login` and returns to `GET /user_api/oidc/callback`. The Worker validates state, nonce, S256 PKCE, discovery/JWKS, ES256 signature, issuer, audience, and expiry; first sign-in requires a verified email and later identity resolution uses OIDC `sub`, not email.
